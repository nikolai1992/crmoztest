...
export default {
    ...
    created() {
        EventBus.on('openEditDealModal', (data) => {
            this.dealId = data.id;
            this.getDeal();
        });
    },
    ...
    methods: {
        getDeal () {
            axios.get('/api/deal/' + this.dealId, []).then(response => {
        ...
        sendRequest () {
            this.resetErrors();
            axios.patch('/api/deal/' + this.dealId, this.formData).then(response => {
                if (response.data.error == null) {
                    this.formData = this.$options.data().formData;
                    this.closeModal();
                    EventBus.emit('dealWasUpdated');
                } else {
